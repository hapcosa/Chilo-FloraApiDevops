...
export function Postulaciones() {
  ...
  const cargar = useCallback(() => {
    ...
    api
      ...
      .catch((fallo) =>
        setError(fallo instanceof Error ? fallo.message : 'No se pudieron cargar las postulaciones'),
      )
  ...
  async function resolver(postulacion: Postulacion, decision: 'aprobada' | 'rechazada') {
    setError(null);
    setAviso(null);
    ...
    if (decision === 'rechazada') {
      ...
      const escrito = window.prompt('Motivo del rechazo (lo verá quien postuló):');
  ...
  return (
    <section>
      <header className="cabecera-seccion">
        ...
        <label>
          Estado
          <select value={estado} onChange={(e) => setEstado(e.target.value as PostulacionEstado)}>
      ...
      <p className="ayuda">
        Aprobar añade al usuario como curador de esa categoría. No cambia su rol global: sigue
        siendo una cuenta normal con permiso sobre lo suyo.
      </p>
      ...
      {cargando ? (
        ...
        <p>No hay postulaciones {estado === 'pendiente' ? 'pendientes' : `en estado ${estado}`}.</p>
      ) : (
          ...
          <tbody>
            {postulaciones.map((postulacion) => (
              <tr key={postulacion.id}>
                ...
                {estado === 'pendiente' && (
                  <td>
                    <button type="button" onClick={() => resolver(postulacion, 'aprobada')}>
                      Aprobar
                    </button>
                    <button type="button" onClick={() => resolver(postulacion, 'rechazada')}>
                      Rechazar
                    </button>
                  </td>
                )}
